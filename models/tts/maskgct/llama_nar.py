...
class DiffLlama(LlamaModel):
    def __init__(
        ...
        # FIX 1: Force SDPA (Flash Attention) 
        ...
        def get_layer_config():
            ...
            if hasattr(layer_config, "_attn_implementation"):
    ...
    def _prepare_decoder_attention_mask(
        ...
        # FIX 2: Highly optimized mask creation
        # Creates a boolean mask (B, 1, 1, S) and expands it for free
        mask_2d = attention_mask[:, None, None, :].to(torch.bool)
        mask_4d = mask_2d.expand(bsz, 1, tgt_len, src_len)
        
        # Allocate exactly one float tensor and fill it
        float_mask = torch.zeros_like(mask_4d, dtype=inputs_embeds.dtype)
        float_mask.masked_fill_(~mask_4d, torch.finfo(inputs_embeds.dtype).min)
        return float_mask
