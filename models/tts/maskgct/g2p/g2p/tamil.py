...
# 4. கிரந்த மற்றும் சிறப்பு எழுத்துக்கள் (Point 10 Implementation)
GRANTHA_CONSONANTS = {
    'ஜ': 'dʒ a', 'ஷ': 'ʂ a', 'ஸ': 's a', 'ஹ': 'h a', 'க்ஷ': 'k ʂ a',
    'ஃ': 'x'
}

def apply_contextual_rules(ipa_text):
    """
    தொல்காப்பியம் மற்றும் நன்னூல் அடிப்படையிலான 15 விதிகளை IPA சரத்தில் பயன்படுத்துதல்.
    """
    
    # விதி 4: இரட்டிப்பு வல்லினம் (Gemination)
    ipa_text = re.sub(r'k\s+k', 'kː', ipa_text)
    ipa_text = re.sub(r's\s+s', 'tʃː', ipa_text) # ச்ச -> tʃː
    ipa_text = re.sub(r'ʈ\s+ʈ', 'ʈː', ipa_text)
    ipa_text = re.sub(r't̪\s+t̪', 't̪ː', ipa_text)
    ...
    # விதி 2: உயிரிடை வல்லினம் நாதமாதல் (Intervocalic voicing)
    vowels = r'(a|aː|i|iː|u|uː|e|eː|o|oː|aɪ|aʊ)'
    ...
    # விதி 8 / Point 13: குற்றியலுகரம் (Word-final unrounded 'u')
    plosives = r'(k|ɡ|tʃ|dʒ|ʈ|ɖ|t̪|d̪|p|b|r)'
    ipa_text = re.sub(rf'{plosives}\s+u\b', r'\1 ʉ', ipa_text) # ʉ added based on user audit
...
def tamil_to_ipa(text, text_tokenizer=None):
    """
    தமிழ் உரையை IPA ஒலியன்களாக மாற்றும் முதன்மைச் சார்பு.
    இங்கு Amphion கட்டமைப்பிற்கான | மற்றும் |_| பிரிப்பான்கள் சேர்க்கப்படுகின்றன.
    """
    if type(text)!= str:
        print(f"DEBUG: Tamil text normalization list input: {text}")
        for i in range(len(text)):
            text[i] = tamil_to_ipa(text[i], text_tokenizer)
        return text
    ...
    for word in words:
        ...
        while i < len(chars):
            char = chars[i]
            if char in TAMIL_VOWELS:
                word_ipa += TAMIL_VOWELS[char] + " "
            elif char in TAMIL_CONSONANTS or char in GRANTHA_CONSONANTS:
                base_phone = TAMIL_CONSONANTS.get(char, GRANTHA_CONSONANTS.get(char))
                if i + 1 < len(chars) and chars[i+1] in VOWEL_MARKS:
                    vowel_mark = chars[i+1]
                    if vowel_mark == '்':
                        word_ipa += base_phone.split(' ') + " "
                    else:
                        word_ipa += base_phone.split(' ') + " " + VOWEL_MARKS[vowel_mark] + " "
                    i += 1
                else:
                    word_ipa += base_phone + " "
            else:
        ...
        word_ipa = apply_contextual_rules(word_ipa)
        ipa_result.append(word_ipa)
        
    # வார்த்தைகளை ஒன்றிணைத்தல் (Amphion word separator '|_|')
    final_ipa = "|_|".join(ipa_result)
    
    # இடைவெளிகளை ஒலியன் பிரிப்பானாக ('|') மாற்றுதல்
    final_ipa = re.sub(r'\s+', '|', final_ipa)
    
    # தொடர்ச்சியான பிரிப்பான்களைச் சரிசெய்தல் மற்றும் ஓரங்களில் உள்ளவற்றை நீக்குதல்
    final_ipa = re.sub(r'\|+', '|', final_ipa).strip('|')
