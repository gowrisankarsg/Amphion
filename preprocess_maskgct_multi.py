...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Single-script pipeline for MaskGCT T2S fine-tuning (now multi-GPU).
...
Multi-GPU support:
  • Automatically detects 2 T4s via torch.cuda.device_count()
  • Wraps the GPU work (W2V-BERT + codec) in nn.DataParallel
  • Keep batch-size at 80 so each GPU handles ~40 utterances (same as your
    single-GPU run).
...
━━━━━━━
  python preprocess_maskgct.py \
    ...
    --batch-size 80 --workers 4 --val-ratio 0.01
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# Semantic pipeline (wrapped for DataParallel)
# ══════════════════════════════════════════════════════════════════════════════

class SemanticPipeline(nn.Module):
    """
    Bundles W2V-BERT layer-17 extraction + mean/std normalisation + VQ codec
    into one nn.Module so nn.DataParallel can shard the batch across GPUs.
    """

    def __init__(self, semantic_model, semantic_mean, semantic_std, semantic_codec):
        super().__init__()
        self.semantic_model = semantic_model
        # Register as buffers so they move/scatter with the module correctly
        self.register_buffer("semantic_mean", semantic_mean)
        self.register_buffer("semantic_std", semantic_std)
...
class SemanticExtractor:
    ...
    def __init__(self, stats_path: Path, codec_ckpt: Optional[Path],
        ...
        # Master device is always cuda:0 when DataParallel is used
        self.device = device
        ...
        safetensors.torch.load_model(
            semantic_codec, str(codec_ckpt), device=str(device)
        )
        semantic_codec = semantic_codec.to(device).eval()

        # Wrap the GPU work in one nn.Module
        self.pipeline = SemanticPipeline(
            semantic_model, semantic_mean, semantic_std, semantic_codec
        ).to(device).eval()

        # DataParallel across every visible CUDA device
        if torch.cuda.device_count() > 1:
            gpu_ids = list(range(torch.cuda.device_count()))
            print(f"[DataParallel] Using GPUs {gpu_ids}")
            self.pipeline = nn.DataParallel(self.pipeline, device_ids=gpu_ids)
            # DataParallel master is always cuda:0
            self.device = torch.device("cuda:0")

        print(f"[SemanticExtractor] Ready — master device={self.device}")
    ...
    def extract_batch(self, waveforms: List[torch.Tensor]) -> np.ndarray:
        ...
        # DataParallel automatically scatters/gathers across GPUs
        semantic_code = self.pipeline(input_features, attention_mask)
...
def process_manifest(
    ...
    print(f"  [Pass 1/2] done — written={n_written:,}  "
          f"already_done={stats['already_done']:>8,}  "
          f"skipped={stats['skipped_g2p']:>8,}")

    # ══════════════════════════════════════════════════════════════════════════
    # PASS 2 — Semantic codes  (GPU, batched, DataParallel)
    # ══════════════════════════════════════════════════════════════════════════
